...
router.post('/', verifyToken,async (req, res) => {
	...
	try {
		if(!body.content){
			...
			res.success({data:item, message:"Privacy policy added successfuly"})
		}
	} catch (err) {
...
router.put('/:id',verifyToken, upload.single("file"),  async (req, res) => {
	...
	try {
		const findPrivacyPolicy = await getSingleItemById(TABLE_NAME, id)
		console.log('findPrivacyPolicy',findPrivacyPolicy);
		if(findPrivacyPolicy.Item){
			const itemObject = {
				content:body.content,
				updatedDate:new Date().toISOString()
			}
			const updated = await updateItem(TABLE_NAME, id, itemObject)
			res.success({data:updated.Attributes, message:"Privacy policy updated successfuly"})
		}else{
		res.errors({message:'privacypolicy not found',data:{}})
		}
