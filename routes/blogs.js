...
router.post('/', verifyToken, upload.single("file"), async (req, res) => {
	...
	try {
		if(!req.file){
			...
			res.success({data:item, message:"Blogs added successfuly"})
		}
	} catch (err) {
...
router.put('/:id',verifyToken, upload.single("file"),  async (req, res) => {
	...
	try {
		const findBlogs = await getSingleItemById(TABLE_NAME, id)
		console.log('findBlogs',findBlogs);
		if(findBlogs.Item){
			const data = findBlogs.Item
			...
			res.success({data:updated.Attributes,message:"Blogs updated successfuly"})
		}else{
		res.errors({message:'blogs not found',data:{}})
		}
